...
func TestCheckOnlyFindsMatchingPlatform(t *testing.T) {
	...
	if result.LatestVersion != "0.2.0" || result.URL != srv.URL+"/downloads/portlight-linux-amd64" {
...
func TestRunSkipsAlreadyLatestVersion(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `{"version":"0.2.0","files":[{"os":"linux","arch":"amd64","url":"/downloads/portlight","sha256":"abc"}]}`)
	}))
	defer srv.Close()
...
func TestRunDownloadsVerifiesAndApplies(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case LatestPath:
			fmt.Fprintf(w, `{"version":"0.2.0","files":[{"os":"linux","arch":"amd64","url":"/downloads/portlight","sha256":"%x"}]}`, sum)
...
func TestRunRejectsChecksumMismatch(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case LatestPath:
			fmt.Fprint(w, `{"version":"0.2.0","files":[{"os":"linux","arch":"amd64","url":"/downloads/portlight","sha256":"0000"}]}`)
		case "/downloads/portlight":
